...
def rollout(env, agent, max_path_length=np.inf, accum_context=True, animated=False, save_frames=False,ids=None):
    """
    ...
    while path_length < max_path_length:
        #print(ids,agent)
        a, agent_info = agent.get_action(o,ids)
        #if ids is not None:
        #    a, agent_info = agent.get_action(o,ids)
        #else:
        #    a, agent_info = agent.get_action(o)
        next_o, r, d, env_info = env.step(np.clip(a,-1.0,1.0))
        # update the agent's current context
        #print(next_o.shape,a.shape,o.shape)
        if accum_context:

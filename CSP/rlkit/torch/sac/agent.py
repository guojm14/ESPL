...
class PEARLAgent(nn.Module):
    ...
    def sample_z(self):
        if self.use_ib:
            ...
            self.z = self.z_means
        # print(self.z.shape,self.z.device)
        
        self.policy.symbolic.update_const(self.z)

    def get_action(self, obs, deterministic=False,ids=None):
        ...
        if self.use_hyper_net:
            ...
            if ids is None:
                ...
                return self.policy.get_action(obs, deterministic=deterministic,ids=ids)
    ...
    def get_policy(self,obs):
        ...
        #     print('policy2_time',start.elapsed_time(end)) 
    ...
    def forward(self, obs, context):
        ...
        #     end.record()
        #     torch.cuda.synchronize()
        #     print('context_time',start.elapsed_time(end)) 
        # run policy, get log probs and new actions
        # if self.policy.mode==1:
        #     start = torch.cuda.Event(enable_timing=True)
        #     end = torch.cuda.Event(enable_timing=True)
        ...
        #     end.record()
        #     torch.cuda.synchronize()
        #     print('policy_time',start.elapsed_time(end)) 
        return policy_outputs, task_z
    ...
    def get_output(self, obs, context, infer_z=True,ids=None):
        ''' given context, get statistics under the current policy of a set of observations '''
        if infer_z:
            self.infer_posterior(context)
            self.sample_z()
        ...
        # run policy, get log probs and new actions
        if ids is None:
            in_ = torch.cat([obs_, task_z.detach()], dim=1)
            policy_outputs = self.policy(in_, reparameterize=True, return_log_prob=True)
        else:
            policy_outputs = self.policy(obs_, reparameterize=True, return_log_prob=True,ids=ids)
